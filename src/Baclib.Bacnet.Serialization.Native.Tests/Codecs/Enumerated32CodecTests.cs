...
{
    [Theory]
    [InlineData(new byte[] { 0x21, 0x2A }, (Enumerated32)42)]
    [InlineData(new byte[] { 0x22, 0x01, 0x00 }, (Enumerated32)256)]
    [InlineData(new byte[] { 0x23, 0x01, 0x00, 0x00 }, (Enumerated32)65536)]
    [InlineData(new byte[] { 0x24, 0xFF, 0xFF, 0xFF, 0xFF }, (Enumerated32)uint.MaxValue)]
    public void Decode_ApplicationTagged_ReturnsExpected(byte[] bytes, Enumerated32 expected)
    {
        var reader = new NativeReader(bytes);
        Assert.Equal(expected, Enumerated32Codec.Instance.Decode(ref reader));
    }
    ...
    public void Decode_ContextTagged_ReturnsExpected()
    {
        ...
        Assert.Equal((Enumerated32)42, Enumerated32Codec.Instance.Decode(ref reader, tagNumber: 0));
    }
    ...
    public void DecodeOptional_PresentValue_ReturnsExpected()
    {
        ...
        var reader = new NativeReader([0x91, 0x2A]);
        Optional<Enumerated32> result = Enumerated32Codec.Instance.DecodeOptional(ref reader);
