...
public class Unsigned16CodecTests
{
    ...
    public void Decode_ApplicationTagged_ReturnsExpected(byte[] bytes, ushort expected)
    {
        ...
        Assert.Equal(expected, Unsigned16Codec.Instance.Decode(ref reader));
    }
    ...
    public void Decode_ContextTagged_ReturnsExpected()
    {
        ...
        var reader = new NativeReader([0x0A, 0x01, 0x00]);
        Assert.Equal((ushort)256, Unsigned16Codec.Instance.Decode(ref reader, tagNumber: 0));
    }

    [Fact]
    public void DecodeOptional_PresentValue_ReturnsExpected()
    {
        var reader = new NativeReader([0x22, 0x03, 0xE8]);
        Optional<ushort> result = Unsigned16Codec.Instance.DecodeOptional(ref reader);
