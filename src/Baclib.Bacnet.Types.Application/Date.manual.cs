...
/// </remarks>
public readonly partial record struct Date
{
    /// <summary>
    ...
    public Date(ReadOnlySpan<byte> data)
    {
        if (data.Length < 4)
        {
    ...
    /// </summary>
    /// <param name="dateTime">The DateTime to convert.</param>
    public static implicit operator Date(System.DateTime dateTime) => new(dateTime);

    /// <summary>
    /// Implicitly converts a .NET DateOnly to a BACnet Date.
    /// </summary>
    /// <param name="date">The DateOnly to convert.</param>
    public static implicit operator Date(DateOnly date) => new(date);
}
