...
public class BitFlags8Tests
{
    [Fact]
    public void Constructor_WithValidCount_ShouldCreateInstance()
    {
        // Arrange & Act
        var bitFlags = new BitString8(0b1010, 4);

        // Assert
        Assert.Equal(4, bitFlags.Count);
        Assert.Equal((byte)0b1010, bitFlags.Flags);
    }

    [Fact]
    public void Constructor_WithCountGreaterThan8_ShouldThrowArgumentOutOfRangeException()
    {
        // Act & Assert
        Assert.Throws<ArgumentOutOfRangeException>(() => new BitString8(0, 9));
    }
    ...
    {
        // Arrange & Act
        var bitFlags = new BitString8(0b11111111, 4); // Pass 8 bits but only use 4

        // Assert
        Assert.Equal(4, bitFlags.Count);
        Assert.Equal((byte)0b1111, bitFlags.Flags); // Only lower 4 bits should remain
    }

    [Fact]
    public void Constructor_WithCount8_ShouldKeepAllBits()
    {
        // Arrange & Act
        var bitFlags = new BitString8(0xFF, 8);

        // Assert
        Assert.Equal(8, bitFlags.Count);
        Assert.Equal((byte)0xFF, bitFlags.Flags);
    }
    ...
    {
        // Arrange & Act
        var bitFlags = new BitString8(0xFF, 0);

        // Assert
        Assert.Empty(bitFlags);
        Assert.Equal((byte)0, bitFlags.Flags);
    }
    ...
    {
        // Arrange
        var bitFlags = new BitString8(0b1010, 4); // bits: 0,1,0,1 (LSB to MSB)
    ...
    {
        // Arrange
        var bitFlags = new BitString8(0b1010, 4); // bits: 0,1,0,1 from LSB to MSB
    ...
    {
        // Arrange
        var bitFlags = new BitString8(0, 0);
    ...
    {
        // Arrange
        var bitFlags1 = new BitString8(0b1010, 4);
        var bitFlags2 = new BitString8(0b1010, 4);
    ...
    public void Equality_WithDifferentFlags_ShouldNotBeEqual()
    {
        // Arrange
        var bitFlags1 = new BitString8(0b1010, 4);
        var bitFlags2 = new BitString8(0b1100, 4);
