...
public readonly record struct AsduTag
{
    ...
    public static int PeekTag(ReadOnlySpan<byte> source, byte tagNumber, AsduTagClass tagClass, out int dataLength)
    {
        ...
        if (lengthValue == 5)
        {
            ...
            if (dataLength == 254)
            {
                ...
                dataLength = BinaryPrimitives.ReadUInt16BigEndian(source.Slice(index + 1, 2));
                return index + 3;
            }
            ...
            dataLength = BinaryPrimitives.ReadInt32BigEndian(source.Slice(index + 1, 4));
            return index + 5;
        }
    ...
    public static int ReadTag(ReadOnlySpan<byte> source, out byte tagNumber, out AsduTagClassType tagClassType, out int dataLength)
    {
        ...
        if (dataLength == 5)
        {
            ...
            if (dataLength == 254)
            {
                dataLength = BinaryPrimitives.ReadUInt16BigEndian(source.Slice(index, 2));
                return index + 2;
            }

            dataLength = BinaryPrimitives.ReadInt32BigEndian(source.Slice(index, 4));
            return index + 4;
        }
    ...
    public static int WriteTag(Span<byte> destination, byte tagNumber, AsduTagClass tagClass, int dataLength)
    {
        ...
        else
        {
            ...
            else if (dataLength < 0x10000)
            {
                ...
                BinaryPrimitives.WriteUInt16BigEndian(destination[index..], unchecked((ushort)dataLength));
            }
            else
            {
                destination[index++] = 255;
                BinaryPrimitives.WriteUInt32BigEndian(destination[index..], unchecked((uint)dataLength));
            }
        }
