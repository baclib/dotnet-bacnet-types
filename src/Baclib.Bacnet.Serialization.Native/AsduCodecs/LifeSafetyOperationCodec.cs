...
/// <summary>
/// Provides BACnet ASDU primitive decoding and encoding for <see cref="T.LifeSafetyOperation"/> values.
/// </summary>
public sealed class LifeSafetyOperationCodec :
    IAsduElementCodec<T.LifeSafetyOperation>,
    IAsduPrimitiveCodec<T.LifeSafetyOperation>
{
    /// <summary>
    /// Decodes an <see cref="T.LifeSafetyOperation"/> value from the current reader position using the application tag.
    /// </summary>
    ...
    public static T.LifeSafetyOperation DecodeValue(ReadOnlySpan<byte> source)
    {
        return source.Length switch
        {
            AsduLength.Enumerated8 => (T.LifeSafetyOperation)AsduBinaryPrimitives.ReadUnsigned8(source),
            AsduLength.Enumerated16 => (T.LifeSafetyOperation)AsduBinaryPrimitives.ReadUnsigned16(source),
            _ => throw new ArgumentOutOfRangeException(nameof(source))
        };
    }

    /// <summary>
    /// Encodes an <see cref="T.LifeSafetyOperation"/> value using the application tag.
    /// </summary>
    ...
    public static void EncodeValue(Span<byte> destination, in T.LifeSafetyOperation value)
    {
    ...
    /// <summary>
    /// Gets the encoded payload length for an <see cref="T.LifeSafetyOperation"/> value.
    /// </summary>
    /// <param name="value">The value whose payload length is requested.</param>
    /// <returns>The encoded payload length in bytes.</returns>
    public static int GetEncodedValueLength(in T.LifeSafetyOperation value)
